...
const STYLES: Record<string, Record<string, string>> = {
  ...
  item: {
    container:
      'rounded-full flex items-center justify-center absolute bg-black/20 backdrop-blur-xl border border-white/10 hover:bg-white/20 cursor-pointer',
  ...
  const closeAnimation = async () => {
    ...
    for (let i = 0; i < scaleTransition.length; i++) {
      await animate.start({
        ...
        backgroundColor: `rgba(255, 255, 255, ${Math.max(0.2 - i * 0.02, 0.1)})`,
        transition: {
